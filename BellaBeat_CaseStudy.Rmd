...
activity <- read.csv("dailyActivity_merged.csv", sep=",") %>%
...
# Read in sleep data
sleep <- read.csv("sleepDay_merged.csv", sep=",")
...
calories_hr <- read.csv("hourlyCalories_merged.csv") %>%
...
steps_hr <- read.csv("hourlySteps_merged.csv") %>%
  mutate(ActivityHour = mdy_hms(ActivityHour),
         Date = format(as.POSIXct(ActivityHour), format="%Y-%m-%d"),
         Time = format(as.POSIXct(ActivityHour), format="%H:%M:%S")) %>%
